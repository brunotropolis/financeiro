...
import type { CartaoCredito, Categoria, ContaBancaria, Entidade, Fornecedor, ProjetoRow, Recorrencia } from "@/lib/types/database";
...
export default async function RecorrenciasPage() {
  ...
  const [recRes, entRes, catRes, fornRes, cartRes, contaRes, bktRes, projRes] = await Promise.all([
    ...
    db.from("categorias").select("id,nome,tipo,cor_hex,ativo,projeto_padrao_id").eq("ativo", true).order("nome"),
  ...
  return (
    <RecorrenciasClient
      ...
      categorias={(catRes.data ?? []) as Pick<Categoria, "id" | "nome" | "tipo" | "cor_hex" | "ativo" | "projeto_padrao_id">[]}
